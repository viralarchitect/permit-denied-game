...
func TestAtlasLoad(t *testing.T) {
	...
	if len(a.ground) != scenario.Map().Height || len(a.decal) != scenario.Map().Height {
		t.Fatalf("lot height: ground=%d decal=%d want %d", len(a.ground), len(a.decal), scenario.Map().Height)
	}
	if len(a.ground[0]) != scenario.Map().Width || len(a.decal[0]) != scenario.Map().Width {
		t.Fatalf("lot width: ground=%d decal=%d want %d", len(a.ground[0]), len(a.decal[0]), scenario.Map().Width)
	}
...
func TestDrawWorldSmoke(t *testing.T) {
	...
	DrawWorld(dst, View{
		...
		Heavies: []threats.Heavy{
			threats.FireTruck(220, 240, 28, 16, 22),
			threats.Wagon(260, 252, 48, 20, 36),
		},
	})
	DrawTitle(dst, 0, 0)
}
...
func TestDrawBurstMissingFrameIsSilent(t *testing.T) {
	...
	drawBursts(dst, View{
		CamX: 0, CamY: 64,
		Bursts: []fx.Burst{{X: 160, Y: 264, Kind: fx.BurstBoom}},
	}, a)
}
...
func TestDrawBoomOnSheriffCrop(t *testing.T) {
	...
	l := scenario.Lot()
	sheriff := l.BuildingByID(lot.TargetSheriff)
	if sheriff == nil {
		t.Fatal("missing sheriff")
	}
	cx, cy := sheriff.Center()
	d := dozer.Spawn(cx, cy+20)
	v := View{
		CamX: 0, CamY: 64,
		Dozer:     d,
		Buildings: l.Buildings,
		Bursts:    []fx.Burst{{X: cx, Y: cy, Kind: fx.BurstBoom}},
	}
	DrawWorld(dst, v)
}
